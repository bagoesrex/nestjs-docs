...
Once these are installed, we can add a few decorators to the `CreateKeonkDto` class. Here we see a significant advantage of this technique: the `CreateKeonkDto` class remains the single source of truth for our Post body object (rather than having to create a separate validation class).
...
> **Hint**
> As a reminder, you don't have to build a generic validation pipe on your own since the `ValidationPipe` is provided by Nest out-of-the-box. The built-in `ValidationPipe` offers more options than the sample we built in this chapter, which has been kept basic for the sake of illustrating the mechanics of a custom-built pipe.
...
@Post()
async create(
  @Body(new ValidationPipe()) createKeonkDto: CreateKeonkDto,
) {
  this.keonksService.create(createKeonkDto);
}
```
...
async findAll(
  ...
  return this.keonksService.findAll({ activeOnly, page });
}
